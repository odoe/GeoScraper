class EsriToGeo:
  ...
  def convert_to_geo(self, esrijson):
    geojson = {}
    ...
    for feat in features:
      feats.append(self.extract(feat, esri_geom_type))

    geojson["features"] = feats

    return geojson

  def extract(self, feature, esri_geom_type):
    item = {}
    item["type"] = "Feature"
    ...
    geometry = {}
    geometry["type"] = self.get_geom_type(esri_geom_type)
    geometry["coordinates"] = self.get_coordinates(geom, geometry["type"])
  ...
  def get_geom_type(self, esri_type):
  ...
  def get_coordinates(self, geom, geom_type):
